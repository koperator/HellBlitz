// --- START OF FILE mercenary.js ---
// Assumes globals: Math, console, distance, normalizeVector, getRandomInt, hasLineOfSight, TILE_SIZE, FOG_STATE, level, performance, window (for frameCount, playSound, imgMercenary)
// Assumes constants: All constants from constants.js (e.g., MERC_*, TILE_SIZE, MERC_WEAPON, SCREAMER_ABILITY_DURATION, SLOW_EFFECT_COLOR, MERC_RADIUS_LOGIC, MERC_DRAW_SCALE, MERC_FALLBACK_COLOR)
...
class Mercenary {
    constructor(x, y, id) {
        this.id = id; 
        this.x = x; this.y = y; 
        this.radius = MERC_RADIUS_LOGIC; // Uses MERC_SIZE which is now 50% larger
        this.hp = MERC_HP;
        this.maxHp = MERC_HP;
    ...
    update(effectiveDt, player, zombies, turrets_ref, mercenaries_ref, isWallFunc, pathfinderGrid, finder, projectiles_ref) {
         ...
         // Adjust follow offset based on the new MERC_SIZE.
         // Since MERC_SIZE is 1.5x, these distance factors might need slight tweaking if they feel too far/close.
         const followOffsetDist = MERC_SIZE * (1.0 + this.id * 0.5); // Was TILE_SIZE based
         ...
         if (this.path.length === 0 || this.targetNodeIndex >= this.path.length || this.failedPathAttempts > 2) { 
             needsNewPath = true;
             ...
             if (pathEndGoalDist > TILE_SIZE * 3.5) { 
                 needsNewPath = true;
             }
         }

         if (distToFormationPos > MERC_FOLLOW_DISTANCE_MAX || (distToFormationPos > TILE_SIZE * 2 && this.path.length === 0 && this.failedPathAttempts > 0) ) { // MERC_FOLLOW_DISTANCE_MAX increased
             needsNewPath = true;
         }
         ...
         if (this.path.length > 0 && this.targetNodeIndex < this.path.length) {
             ...
             if (distNode < TILE_SIZE * 0.7) { 
                 this.targetNodeIndex++;
                 if (this.targetNodeIndex >= this.path.length) { this.path = []; }
             }
         }

         if (!movingAlongPath && distToFormationPos > MERC_FOLLOW_DISTANCE_MIN) { // MERC_FOLLOW_DISTANCE_MIN increased
         ...
         mercenaries_ref.forEach(other => {
            if(other !== this && other.active) {
                ...
                if(distOther > 0.1 && distOther < MERC_SEPARATION_DISTANCE){ // MERC_SEPARATION_DISTANCE increased
         ...
         if (this.targetZombie && !this.reloading && this.fireTimer <= 0) {
             ...
             const barrelOffset = this.radius * 1.1; // Barrel offset scales with radius
    ...
    draw(ctx, offsetX, offsetY) {
        ...
        if (typeof level !== 'undefined' && level.fogGrid?.[gridY]?.[gridX] === FOG_STATE.REVEALED) {
            ...
            const mercDrawSize = this.radius * 2 * MERC_DRAW_SCALE; // MERC_DRAW_SCALE is 1.0, this.radius is now larger
            if (window.imgMercenary && window.imgMercenary.complete && window.imgMercenary.naturalHeight !== 0) {
                ctx.drawImage(window.imgMercenary, -mercDrawSize / 2, -mercDrawSize / 2, mercDrawSize, mercDrawSize);
            } else {
                 ctx.fillStyle = (typeof MERC_FALLBACK_COLOR !== 'undefined' ? MERC_FALLBACK_COLOR : '#2d47cf');
             ...
             const barX = this.x - offsetX - barWidth / 2; const barY = this.y - offsetY - this.radius - barHeight - 4; // Adjusted y for larger radius
