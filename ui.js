...
// Assumes window.player, window.kills, window.elapsedTime, window.zombies, window.turrets, window.powerupDisplayMessages, imgCharSelectBg, imgGameOver, imgYouWin (from game_setup.js)
// Assumes constants: classes, weapons, TurretState, ENGINEER_MAX_TURRETS, CLASS_ID, WEAPON_ID, PSION_SHIELD_HP, PSI_BLAST_MIN_SHIELD_COST, ENGINEER_DRONE_COUNT, POWERUP_MESSAGE_BASE_DURATION
...
const MAX_MESSAGE_WIDTH = typeof canvas !== 'undefined' ? canvas.width * 0.5 : 400; 
...
function drawCharacterSelection() {
    ...
    const classNameFontSize = Math.floor(boxHeight * 0.15); 
    const descFontSize = Math.floor(boxHeight * 0.12);    
    const statFontSize = Math.floor(boxHeight * 0.11);    
    ...
    classes.forEach((cls, index) => {
        ...
        const descLines = wrapText(ctx, shortDesc.toUpperCase(), boxWidth - textBlockPadding * 2); 
        ...
        ctx.fillText(hpStat.toUpperCase(), boxX + textBlockPadding, currentTextY); 
        currentTextY += statLineHeight;

        const weapon = weapons.find(w => w.id === cls.weaponId);
        ctx.fillText(`WPN: ${weapon ? weapon.name.toUpperCase() : 'N/A'}`, boxX + textBlockPadding, currentTextY); 
    });
     ctx.textAlign = 'left';
     ctx.lineWidth = 1;
}
...
function drawGameOver() {
    // Draw the game over background image first
    if (window.imgGameOver && window.imgGameOver.complete && window.imgGameOver.naturalHeight !== 0) {
        ctx.drawImage(window.imgGameOver, 0, 0, canvas.width, canvas.height);
    } else {
        // Fallback if image isn't loaded
        ctx.fillStyle = 'rgba(0, 0, 0, 0.9)'; // Darker overlay if image fails
        ctx.fillRect(0, 0, canvas.width, canvas.height);
    }
    ...
    // Ensure text has a shadow or outline for readability over a potentially complex image
...
function drawGameWon() {
    // Draw the you win background image first
    if (window.imgYouWin && window.imgYouWin.complete && window.imgYouWin.naturalHeight !== 0) {
        ctx.drawImage(window.imgYouWin, 0, 0, canvas.width, canvas.height);
    } else {
        // Fallback if image isn't loaded
        ctx.fillStyle = 'rgba(0, 20, 0, 0.9)'; // Dark green overlay
        ctx.fillRect(0, 0, canvas.width, canvas.height);
    }
...
function drawUI() {
    ...
    if (weapon.id !== WEAPON_ID.PSI_BLADES) {
        let ammoText = `AMMO: ${player.ammo} / ${weapon.magSize === Infinity ? '∞' : weapon.magSize}`; 
        let ammoColor = 'white';
        if (player.reloading) {
        ...
        ctx.fillStyle = 'white';
        ctx.fillText(`${weapon.name.toUpperCase()}`, canvas.width - 15, canvas.height - 70); 
    } else {
        ctx.fillStyle = 'white';
        ctx.fillText(`${weapon.name.toUpperCase()}`, canvas.width - 15, canvas.height - 70); 
    }
    ...
    if (player.classData.id === CLASS_ID.DEVASTATOR && player.abilityType === 'rpg') abilityName = `RPG (${player.abilityUsesLeft} LEFT)`; 
    else if (player.isBrawler && player.abilityType === 'dash') abilityName = 'DASH'; 
    else if (player.isRecon && player.abilityType === 'bullet_time') abilityName = 'BULLET TIME'; 
    else if (player.isEngineer && player.abilityType === 'turret') abilityName = 'MANAGE TURRETS'; 
    else if (player.isPsion && player.abilityType === 'psi_blast') abilityName = 'PSI BLAST'; 

    let abilityText = `ABILITY: ${abilityName}`; let abilityColor = 'white'; 
    ...
    if (player.isBrawler) {
        ...
        if (player.abilityCharges < player.abilityMaxCharges && player.abilityRechargeTimer > 0) {
            abilityText += ` ( ${(player.abilityRechargeTimer / 1000).toFixed(1)}S)`; 
            abilityColor = 'yellow';
        ...
        if (player.abilityCooldownTimer > 0 && !player.isBulletTimeActive) { abilityText += ` (CD ${(player.abilityCooldownTimer / 1000).toFixed(1)}S)`; abilityColor = 'yellow';} 
        else { abilityColor = 'lime'; }
    } else if (player.isPsion) {
        if (player.abilityCooldownTimer > 0) { abilityText += ` (CD ${(player.abilityCooldownTimer / 1000).toFixed(1)}S)`; abilityColor = 'yellow'; } 
        else if (player.shieldHp < PSI_BLAST_MIN_SHIELD_COST) { abilityText += ` (NEED ${PSI_BLAST_MIN_SHIELD_COST} SH)`; abilityColor = 'grey'; } 
        else { abilityColor = 'lime'; }
        if (player.isCastingPsiBlast) { abilityText += " (CAST...)"; abilityColor = 'yellow'; } 
    } else if (player.isEngineer) {
        if (player.abilityCooldownTimer > 0) { abilityText += ` (CD ${(player.abilityCooldownTimer / 1000).toFixed(1)}S)`; abilityColor = 'yellow'; } 
        else { const activeTurrets = window.turrets.filter(t => t.owner === player && (t.state === TurretState.ACTIVE || t.state === TurretState.DEPLOYING)).length; if (activeTurrets < ENGINEER_MAX_TURRETS) { abilityText += " (DEPLOY)"; } else { abilityText += ` (RECALL ${player.nextTurretToRecall + 1})`; } abilityColor = 'lime'; } 
        ...
        if (player.abilityCooldownTimer > 0) { abilityText += ` (CD ${(player.abilityCooldownTimer / 1000).toFixed(1)}S)`; abilityColor = 'yellow'; } 
        else if (player.abilityUsesLeft === 0 && player.abilityUsesTotal !== Infinity) { abilityText += " (EMPTY)"; abilityColor = 'grey'; } 
        else { abilityColor = 'lime'; }
    }

    if (player.isBulletTimeActive) { abilityText += ` (ACTIVE ${(player.bulletTimeTimer / 1000).toFixed(1)}S)`; abilityColor = 'cyan'; } 
    ...
    const aliveZombies = window.zombies ? window.zombies.length : 0; ctx.fillText(`ZOMBIES: ${aliveZombies}`, 15, 20); 
    ctx.fillText(`KILLS: ${window.kills}`, 15, 40); 
    const timeMinutes = Math.floor(window.elapsedTime / 60); const timeSeconds = Math.floor(window.elapsedTime % 60);
    ctx.fillText(`TIME: ${timeMinutes.toString().padStart(2, '0')}:${timeSeconds.toString().padStart(2, '0')}`, 15, 60); 
    if (player.isEngineer && window.turrets) { const deployedTurretCount = window.turrets.filter(t => t.owner === player && (t.state === TurretState.ACTIVE || t.state === TurretState.DEPLOYING)).length; ctx.fillText(`TURRETS: ${deployedTurretCount}/${ENGINEER_MAX_TURRETS}`, 15, 80); } 
    ...
    if (window.powerupDisplayMessages && window.powerupDisplayMessages.length > 0) {
        ...
        window.powerupDisplayMessages.forEach((msg, index) => {
            let alpha = 1.0;
            if (msg.initialTimer - msg.timer < FADE_DURATION) {
                alpha = Math.max(0, (msg.initialTimer - msg.timer) / FADE_DURATION);
            } 
            else if (msg.timer < FADE_DURATION) {
            ...
            linesToDraw.forEach((line, lineIndex) => {
                ...
                ctx.fillText(line.toUpperCase(), canvas.width / 2, startYOffset + (lineIndex * msgLineHeight)); 
            });
        });
        ctx.textAlign = 'left';
    }
