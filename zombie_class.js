// --- START OF FILE zombie_class.js ---
// Assumes globals: Math, console, distance, normalizeVector, getRandomElement, hasLineOfSight, TILE_SIZE, FOG_STATE, kills, level, isWall, performance, PF, ZOMBIE_SIZE, Player, Mercenary, Drone, TurretState, OwnerType, Projectile, createExplosion, Shockwave, gameTimeScale, window (for player, zombies, mercenaries, turrets, explosiveCrates, frameCount, playSound, powerups, imgEnemyDrone), ZOMBIE_BASE_RADIUS, zombieSprites (from zombie_sprites.js)
// Assumes constants: All constants from constants.js (e.g., ZOMBIE_TYPE, TANK_*, PowerupType, Powerup)
// Assumes classes: TurretState, Powerup (from powerup.js), ExplosiveCrate
...
class Zombie {
    constructor(x, y, type = ZOMBIE_TYPE.REGULAR) {
        ...
        this.angle = 0; // For drawing enemy drones or other rotating zombies
    ...
    update(effectiveDt, player, mercenaries_ref, turrets_ref, pathfinderGrid, finder, isWallFunc, zombies, projectiles_ref, shockwaves_ref) {
        ...
        if (!this.isHiveMaster) {
            ...
            // Zombies generally don't target explosive crates directly for movement/attack unless specifically programmed.
            // They might be damaged by crate explosions if nearby.
        ...
        if (canPathfindThisFrame && this.pathTimer >= this.pathCooldown && currentTargetForMovement && pathfinderGrid && finder && needsNewPath && (now - this.lastPathRequestTime > 250)) {
            ...
            if (!isWallFunc(startX, startY) && !isWallFunc(endX, endY) && (startX !== endX || startY !== endY)) { // isWallFunc now considers crates
                if (pathfinderGrid) {
                    try {
                        this.path = finder.findPath(startX, startY, endX, endY, pathfinderGrid); // pathfinderGrid treats crates as walkable (they explode)
                        this.targetNodeIndex = 0;
        ...
        } else if (this.attackRange > 0) { // Ranged zombies
        ...
        if (shouldTryMoving && (dx !== 0 || dy !== 0)) {
            if (Math.sqrt(dx*dx + dy*dy) > this.radius * 0.1) { 
                ...
                this.isMoving = true;
                if (this.type === ZOMBIE_TYPE.DRONE ) { 
                    this.angle = Math.atan2(moveY, moveX);
                } else if (this.type === ZOMBIE_TYPE.SPITTER && this.currentTarget) { 
        ...
        if (this.isMoving) {
            ...
            if (!isWallFunc(Math.floor(nextX / TILE_SIZE), Math.floor(this.y / TILE_SIZE))) { this.x = nextX; } // isWallFunc accounts for crates
        ...
        if (!this.isHiveMaster && this.currentTarget && this.currentTarget.hp > 0 && (this.currentTarget.active !== false || (this.currentTarget.state && this.currentTarget.state === TurretState.ACTIVE) || this.currentTarget instanceof Drone) ) {
            ...
            if (canAttack) {
                if (this.type === ZOMBIE_TYPE.SPITTER) { 
                    if (distToFinalTargetSq <= attackTriggerDistSq && hasLineOfSight(this.x, this.y, this.currentTarget.x, this.currentTarget.y, isWallFunc)) {
                ...
                } else if (this.attackRange === 0) { // MELEE general (includes ZOMBIE_TYPE.DRONE here)
                    if (distToFinalTargetSq <= attackTriggerDistSq) {
        ...
        if (this.type === ZOMBIE_TYPE.DRONE || this.type === ZOMBIE_TYPE.SPITTER) {
            if (!this.isMoving && !this.isAttacking && this.currentTarget) {
                 this.angle = Math.atan2(this.currentTarget.y - this.y, this.currentTarget.x - this.x); 
            } else if (!this.isMoving && !this.isAttacking && !this.currentTarget && this.type === ZOMBIE_TYPE.DRONE && this.hiveParent) {
                this.angle = Math.atan2(this.hiveParent.y - this.y, this.hiveParent.x - this.x); 
            }
        } else if (this.type !== ZOMBIE_TYPE.DRONE && this.type !== ZOMBIE_TYPE.SPITTER) {
            if (!this.isMoving) {
                // this.angle = Math.PI / 2; 
            } else if (dx !==0 || dy !== 0) { 
                if (!this.isAttacking) this.angle = Math.atan2(dy, dx);
            }
        }
    ...
    takeDamage(amount, sourceType = 'unknown', stunDuration = null) {
        ...
        if (this.hp <= 0) {
            ...
            if (this.type === ZOMBIE_TYPE.TYRANT) {
                if (typeof Powerup !== 'undefined' && typeof PowerupType !== 'undefined' && window.powerups && typeof powerupConfig !== 'undefined') {
                    const powerupTypeToDrop = PowerupType.GENERIC_BONUS_HP;
                    if (powerupConfig[powerupTypeToDrop]) {
                         window.powerups.push(new Powerup(this.x, this.y, powerupTypeToDrop));
                         console.log(`Tyrant dropped powerup: ${powerupTypeToDrop} at ${this.x.toFixed(0)}, ${this.y.toFixed(0)}`);
                    } else {
                        console.warn("Powerup config missing for Tyrant drop:", powerupTypeToDrop);
                    }
                } else {
                    console.warn("Powerup class, PowerupType, window.powerups or powerupConfig not available for Tyrant drop.");
                }
            }
        }
             ...
             if (typeof createExplosion === 'function') {
                 if (typeof window.player !== 'undefined' && typeof window.zombies !== 'undefined' && typeof window.mercenaries !== 'undefined' && typeof window.turrets !== 'undefined') {
                    createExplosion(
                        ...
                        window.player, window.zombies, window.mercenaries, window.turrets, window.explosiveCrates, // Pass explosiveCrates
                        null, null,
                        [BLOATER_EXPLOSION_COLOR_PRIMARY, BLOATER_EXPLOSION_COLOR_SECONDARY],
                        'BLOATER_EXPLODE', 1.0, true 
                    );
                    ...
                    createExplosion(
                        ...
                        this.explosionDamageMax * 0.5, this.explosionDamageMin * 0.5, 
    ...
    draw(ctx, offsetX, offsetY, playerRef = null) {
        ...
        if (isTileNormallyVisible || isXRayVisible) {
            ...
            let spriteDrawScaleMultiplier = 2.5; 

            if (this.type === ZOMBIE_TYPE.DRONE) {
                spriteToUse = window.imgEnemyDrone; 
                spriteDrawScaleMultiplier = 1.8; 
            } else {
                spriteToUse = zombieSprites[String(this.type)];
            }
            
            const spriteDrawSize = this.radius * spriteDrawScaleMultiplier;


            if (spriteToUse && spriteToUse.complete && spriteToUse.naturalHeight !== 0) {
                ctx.save();
                ...
                const spriteDrawX = -spriteDrawSize / 2; 
                const spriteDrawY = -spriteDrawSize / 2;
                ctx.drawImage(spriteToUse, spriteDrawX, spriteDrawY, spriteDrawSize, spriteDrawSize);
                ...
                if (this.isHiveMaster && isTileNormallyVisible) {
                    ...
                    ctx.arc(0, 0, this.radius * 0.5 * pulseFactor, 0, Math.PI * 2); ctx.fill();
                }
            ...
            if (isTileNormallyVisible && isStunned) {
                ...
                ctx.beginPath(); ctx.arc(0, 0, stunArcRadius, Math.PI * 1.2, Math.PI * 1.8); ctx.stroke();
                ctx.beginPath(); ctx.arc(0, 0, stunArcRadius, Math.PI * 0.2, Math.PI * 0.8); ctx.stroke();
            }
            ...
            if (isTileNormallyVisible) {
                ...
                if (showHealthBarForType) {
                    ...
                    if (spriteToUse && spriteToUse.complete && spriteToUse.naturalHeight !== 0) {
                        topOfEntityVisualY = this.y - offsetY - spriteDrawSize / 2;
                    } else {
                        topOfEntityVisualY = this.y - offsetY - this.radius;
                    }
