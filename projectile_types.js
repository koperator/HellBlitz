...
// Assumes globals: Math, console, distance, normalizeVector, dotProduct, TILE_SIZE, FOG_STATE, window (for level, player, zombies, mercenaries, turrets, explosiveCrates, grenadeParticles, smokeParticles, playSound, temporarySpriteEffects, imgGrenadeExplosionSprite, imgRPGExplosionSprite)
     ...
     explode(explodeX, explodeY) {
         ...
         if (typeof window.grenadeParticles !== 'undefined' && typeof GrenadeParticle !== 'undefined') {
          ...
          if (typeof createExplosion === 'function') {
                createExplosion(
                    explodeX, explodeY, radius, maxDamage, minDamage,
                    GRENADE_PARTICLE_COUNT, 
                    GRENADE_PARTICLE_SPEED, GRENADE_PARTICLE_LIFESPAN,
                    GRENADE_PARTICLE_LENGTH, GRENADE_PARTICLE_WIDTH, GRENADE_PARTICLE_DAMAGE,
                    window.player, window.zombies, window.mercenaries, window.turrets, window.explosiveCrates, 
                    null, null, null, null, 0.0, false 
                );
          } else {
                console.error("createExplosion function not defined, grenade AoE damage will not occur.");
          }
...
class RPGProjectile {
    ...
    update(effectiveDt, isWallFunc) {
        ...
        if (isWallFunc(gridX, gridY)) {
            ...
            if (window.explosiveCrates) entitiesToTest.push(...window.explosiveCrates); 
        ...
        if (!this.active) return;
         if (typeof createExplosion === 'function' && typeof window.player !== 'undefined' && typeof window.zombies !== 'undefined' && typeof window.mercenaries !== 'undefined' && typeof window.turrets !== 'undefined') {
             createExplosion(explodeX, explodeY, RPG_EXPLOSION_RADIUS, RPG_DAMAGE_CENTER, RPG_DAMAGE_EDGE, RPG_EXPLOSION_PARTICLE_COUNT, RPG_EXPLOSION_PARTICLE_SPEED, RPG_EXPLOSION_PARTICLE_LIFESPAN, RPG_PARTICLE_LENGTH, RPG_PARTICLE_WIDTH, 0,
                             window.player, window.zombies, window.mercenaries, window.turrets, window.explosiveCrates, 
                             impactAngle, impactTarget, null, 'RPG_EXPLODE', 1.0);
            ...
            if (typeof TemporarySpriteEffect !== 'undefined' && window.imgRPGExplosionSprite && typeof window.temporarySpriteEffects !== 'undefined') {
                window.temporarySpriteEffects.push(new TemporarySpriteEffect(
                    ...
                    120 * 0.7, // durationMs (reduced by 30%)
                    0.45, 1.0, 
                    Math.random() * Math.PI * 2,
