...
function spawnZombies() {
    ...
    const currentSpitterChance = SPITTER_CHANCE * timeFactor; 
    const currentBloaterChance = BLOATER_CHANCE * timeFactor;
    const currentScreamerChance = SCREAMER_CHANCE * (0.2 + timeFactor * 0.8); 

    let currentHeavyChance = 0;
    let spawnRegulars = true;
    if (window.elapsedTime > 240) { 
        currentHeavyChance = HEAVY_ZOMBIE_CHANCE_LATE;
        spawnRegulars = false; 
    } else if (window.elapsedTime > 120) { 
