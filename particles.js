...
// Assumes globals: Math, console, distance, normalizeVector, dotProduct, lerp, getRandomInt, TILE_SIZE, FOG_STATE, performance, level, player, zombies, mercenaries, turrets, explosiveCrates, _playerRef, _zombieArr, _mercArr, _turretArr
// Assumes constants: All constants from constants.js are global (including RPG_EXPLOSION_PARTICLE_LIFESPAN)
// Assumes classes: TurretState (imported or global), ExplosiveCrate (will be defined)
...
class PsiBlastParticle {
    ...
    update(effectiveDt, isWallFunc, zombies) { // zombies is passed but we also check window.explosiveCrates
        if (!this.active) return; 
        const dampingFactor = Math.max(0, 1.0 - (this.damping * effectiveDt)); 
        this.vx *= dampingFactor; this.vy *= dampingFactor; 
        ...
        if (isWallFunc(gridX, gridY)) { this.active = false; return; } 
        if (typeof level !== 'undefined' && (this.x < 0 || this.x > level.width * TILE_SIZE || this.y < 0 || this.y > level.height * TILE_SIZE)) { this.active = false; return; } 
            ...
            for (const zombie of zombies) { 
                if (zombie && zombie.hp > 0 && !this.hitTargets.has(zombie)) { 
                    if (distance(this.x, this.y, zombie.x, zombie.y) < this.radius + zombie.radius) { 
                        if (this.damage > 0) { 
                            zombie.takeDamage(this.damage, 'psi_blast', this.stunDuration); 
                        } 
        ...
        // Check collision with explosive crates
        if (window.explosiveCrates && Array.isArray(window.explosiveCrates)) {
            const ExplosiveCrateClass = typeof ExplosiveCrate !== 'undefined' ? ExplosiveCrate : null;
            for (const crate of window.explosiveCrates) {
                if (ExplosiveCrateClass && crate instanceof ExplosiveCrateClass && crate.active && !this.hitTargets.has(crate)) {
                    if (distance(this.x, this.y, crate.x, crate.y) < this.radius + crate.radius) {
                        if (this.damage > 0 && typeof crate.takeDamage === 'function') {
                            crate.takeDamage(this.damage, 'psi_blast');
                        }
                        this.hitTargets.add(crate);
                    }
                }
            }
        }
...
class RailgunParticle {
    constructor(x, y) {
        ...
        this.radius = RAILGUN_PARTICLE_RADIUS * (0.8 + Math.random() * 0.4);
        this.lifespan = RAILGUN_PARTICLE_LIFESPAN * (0.77 + Math.random() * 0.87); 
        this.startLifespan = this.lifespan;
        this.active = true;
        this.alpha = 1.0;
        this.color = RAILGUN_PARTICLE_COLOR_TRAIL;
    }
    update(effectiveDt) {
        if (!this.active) return;
        this.lifespan -= effectiveDt * 1000;
        ...
        this.alpha = Math.max(0, this.lifespan / this.startLifespan);
    }
    draw(ctx, offsetX, offsetY) {
        if (!this.active) {
            return;
...
class GrenadeParticle {
    ...
    update(effectiveDt, isWallFunc, zombies, player, mercenaries, turrets) { // Add explosiveCrates if needed
        ...
        const checkCollision = (target) => {
             ...
             else if (target.state && typeof TurretState !== 'undefined') isActive = target.state === TurretState.ACTIVE; // TurretState might not be global here
             else if (ExplosiveCrateClass && target instanceof ExplosiveCrateClass) isActive = target.active;
        ...
        if (window.explosiveCrates && window.explosiveCrates.some(checkCollision)) { this.active = false; return; }
    }
...
class ExplosionParticle extends GrenadeParticle { // Inherits GrenadeParticle's collision logic
    ...
    // Update is inherited from GrenadeParticle, which now checks explosiveCrates
    draw(ctx, offsetX, offsetY) {
        if (!this.active) return;
        const gridX = Math.floor(this.x / TILE_SIZE); const gridY = Math.floor(this.y / TILE_SIZE);
        if (typeof level !== 'undefined' && level.fogGrid?.[gridY]?.[gridX] !== FOG_STATE.REVEALED) return;
...
class FlameParticle {
    ...
    update(effectiveDt, isWallFunc, zombies, turrets) { // Add explosiveCrates if needed
        if (!this.active) return;
        ...
        const checkCollision = (target) => {
            ...
            let isActive = false;
            if (target.state && typeof TurretState !== 'undefined') isActive = target.state === TurretState.ACTIVE;
            else if (ExplosiveCrateClass && target instanceof ExplosiveCrateClass) isActive = target.active;
            else if (target.hp !== undefined) isActive = target.hp > 0;
            ...
            if(isActive) {
                ...
                if (distance(this.x, this.y, target.x, target.y) < currentRadius + targetRadius) {
                    ...
                    else if (target.hp !== undefined) { // Fallback for entities without takeDamage (like crates if they don't have it)
                        target.hp -= FLAME_PARTICLE_DAMAGE;
        ...
        if (window.explosiveCrates && window.explosiveCrates.length > 0) window.explosiveCrates.forEach(checkCollision);
    }
...
class TemporarySpriteEffect {
    constructor(x, y, image, durationMs, startScale = 1.0, endScale = 1.0, rotation = 0, fixedSize = null, initialScaleX = 1.0, initialScaleY = 1.0) { 
        this.x = x;
        this.y = y;
        this.image = image;
    ...
    draw(ctx, offsetX, offsetY, playerAngle = 0) {
        ...
        } else if (this.lifespan < this.durationMs * 0.3) { 
             alpha = this.lifespan / (this.durationMs * 0.3);
        }
        else { 
            alpha = 1.0;
        }
