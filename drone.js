...
class Drone {
    constructor(owner, index) {
        ...
        this.active = true;
        this.hp = 1; 
        this.playedDestroySound = false; 
        this.radius = DRONE_RADIUS_LOGIC; 
        this.angle = 0; 
    ...
    update(effectiveDt, zombies, isWallFunc, projectiles_ref, allDrones) {
        ...
        if (this.target && this.fireCooldown > (1000 / DRONE_FIRE_RATE) * 0.8) { 
        ...
        // Drones prioritize zombies. They do not target explosive crates.
        zombies.forEach(zombie => { 
            ...
            if (distSq < closestDistSq) { 
                ...
                if (typeof level !== 'undefined' && level.fogGrid?.[targetTileY]?.[targetTileX] === FOG_STATE.REVEALED && hasLineOfSight(this.x, this.y, zombie.x, zombie.y, isWallFunc)) { 
                    closestDistSq = distSq; potentialTarget = zombie; 
                } 
            } 
        });

        if (potentialTarget !== this.target && potentialTarget !== null) { 
            const currentTargetDistSq = this.target ? distance(this.x, this.y, this.target.x, this.target.y)**2 : Infinity; 
            if (!this.target || this.target.hp <= 0 || closestDistSq < currentTargetDistSq * 0.7) { 
                this.switchTargetCooldownTimer = DRONE_SWITCH_TARGET_COOLDOWN; this.target = potentialTarget; 
            } 
        }
        else if (this.target && this.target.hp <= 0) { this.target = potentialTarget; } 
        else if (!this.target){ this.target = potentialTarget; }

        if (this.target && this.fireCooldown <= 0 && this.switchTargetCooldownTimer <= 0) {
            const targetAngleProjectile = Math.atan2(this.target.y - this.y, this.target.x - this.x); 
            projectiles_ref.push(new Projectile(
                 this.x, this.y, targetAngleProjectile,
    ...
    draw(ctx, offsetX, offsetY) {
        ...
        const droneDrawSize = this.radius * 2 * DRONE_DRAW_SCALE; 
        ...
        if (window.imgPlayerDrone && window.imgPlayerDrone.complete && window.imgPlayerDrone.naturalHeight !== 0) {
            ctx.rotate(this.angle); 
            ctx.drawImage(window.imgPlayerDrone, -droneDrawSize / 2, -droneDrawSize / 2, droneDrawSize, droneDrawSize);
        } else {
             ctx.fillStyle = (typeof DRONE_FALLBACK_COLOR !== 'undefined' ? DRONE_FALLBACK_COLOR : 'lightblue');
